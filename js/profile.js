async function initProfilePage() {
  ...
  avatar.innerHTML = profile.photo_url ? `<img class="avatar-photo" src="${profile.photo_url}" alt="${profile.name}" loading="eager" decoding="async" />` : `<div class="avatar-initials">${profile.name.split(" ").map((part) => part[0]).join("").slice(0, 3).toUpperCase()}</div>`;
